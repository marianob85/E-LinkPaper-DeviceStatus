...
#define FMT_APPEND( ... )                           \
	...
	lines_.resize( lines_.length() - FMT_BUF_LEN    \
				   + snprintf( &lines_[ lines_.length() - FMT_BUF_LEN ], FMT_BUF_LEN, ##__VA_ARGS__ ) );
...
struct inner
{
	static int http_request(
		const char*, const char*, const std::string&, const std::string&, const server_info&, std::string* );
...
protected:
	...
	int _send_udp( const std::string& host, int port )
	{
		...
		if( sendto( sock, &lines_[ 0 ], lines_.length(), 0, ( struct sockaddr* )&addr, sizeof( addr ) )
			< ( int )lines_.length() )
			ret = -3;
...
void inner::url_encode( std::string& out, const std::string& src )
{
	size_t pos = 0, start = 0;
	while(
		( pos = src.find_first_not_of( "abcdefghijklmnopqrstuvwxyqABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_.~", start ) )
		!= std::string::npos )
	{
		...
		{
			out += '%';
			out += to_hex( ( unsigned char )( ( unsigned char )src[ pos ] >> 4 ) );
			out += to_hex( ( unsigned char )( ( unsigned char )src[ pos ] & 0xF ) );
		}
		start = ++pos;
	}
	out.append( src.c_str() + start, src.length() - start );
}
int inner::http_request( const char* method,
						 const char* uri,
						 ...
						 std::string* resp )
{
	...
	addr.sin_family = AF_INET;
	addr.sin_port   = htons( uint16_t( si.port_ ) );
	...
	for( ;; )
	{
		iv[ 0 ].iov_len = snprintf( &header[ 0 ],
									len,
...
#define __( c )                     \
	...
	_UNTIL( ' ' ) _GET_NUMBER( ret_code ) for( ;; )
	{
		...
		case 'T':
			...
			_( 'n' )
			_( 's' )
			_( 'f' ) _( 'e' ) _( 'r' ) _( '-' ) _( 'E' ) _( 'n' ) _( 'c' ) _( 'o' ) _( 'd' ) _( 'i' ) _( 'n' ) _( 'g' )
				_( ':' ) _( ' ' ) _( 'c' ) _( 'h' ) _( 'u' ) _( 'n' ) _( 'k' ) _( 'e' ) _( 'd' ) chunked
				= true;
			break;
		case '\r':
			__( '\n' )
			switch( chunked )
			{
				...
				{
					__( '\r' )
					__( '\n' ) case true : _GET_CHUNKED_LEN( content_length, '\r' ) __( '\n' ) if( !content_length )
					{
						__( '\r' ) __( '\n' ) goto END;
					}
				case false:
					...
					{
						content_length
							-= ( iv[ 1 ].iov_len = std::min( content_length, ( int )iv[ 0 ].iov_len - len ) );
						if( resp )
