...
static int getFieldValueInCpuInfo( char* hardware, int hardwareMaxLen, char* revision, int revisionMaxLen )
{
	...
	while( !feof( f ) )
	{
		...
		else
		{
			...
			{
				if( lineUntrim[ i ] == ' ' || lineUntrim[ i ] == '\t' || lineUntrim[ i ] == '\r'
					|| lineUntrim[ i ] == '\n' )
				{
				}
				else
...
int getBoardType( BoardHardwareInfo** retBoardInfo )
{
	...
	if( strncasecmp( hardware, a64, strlen( a64 ) ) == 0 || strncasecmp( hardware, amlogic, strlen( amlogic ) ) == 0 )
	{
		for( i = 0; i < ( sizeof( gAllBoardHardwareInfo ) / sizeof( BoardHardwareInfo ) ); i++ )
		{
			if( strncasecmp( gAllBoardHardwareInfo[ i ].kernelHardware,
							 hardware,
							 strlen( gAllBoardHardwareInfo[ i ].kernelHardware ) )
				== 0 )
			{
				if( retBoardInfo != 0 )
				{
		...
		|| strncasecmp( hardware, h5_kernel4, strlen( h5_kernel4 ) ) == 0 )
	{
		...
		if( ret == 0 )
		{
			...
			for( i = 0; i < ( sizeof( gAllBoardHardwareInfo ) / sizeof( BoardHardwareInfo ) ); i++ )
			{
				// LOGD("\t\t enum, start compare[%d]: %s <--> %s\n", i, gAllBoardHardwareInfo[i].kernelHardware,
				// hardware);
				if( strncasecmp( gAllBoardHardwareInfo[ i ].kernelHardware,
								 hardware,
								 strlen( gAllBoardHardwareInfo[ i ].kernelHardware ) )
					== 0 )
				{
					if( strncasecmp( gAllBoardHardwareInfo[ i ].allwinnerBoardID,
									 allwinnerBoardID,
									 strlen( gAllBoardHardwareInfo[ i ].allwinnerBoardID ) )
						== 0 )
					{
						if( retBoardInfo != 0 )
						{
