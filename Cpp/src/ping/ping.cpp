...
unsigned short Ping::getChksum( unsigned short* addr, int len )
{
	...
	sum += ( sum >> 16 );
	answer = static_cast< unsigned short >( ~sum );
	return answer;
}
...
int Ping::packIcmp( int pack_no, struct icmp* icmp )
{
	...
	picmp->icmp_code  = 0;
	picmp->icmp_cksum = 0;
	picmp->icmp_seq   = ( u_int16_t )pack_no;
	picmp->icmp_id	= ( u_int16_t )m_pid;
...
bool Ping::sendPacket()
{
	...
	while( m_nsend < m_maxPacketSize )
	{
		...
		if( sendto( m_sockfd, m_sendpacket, packetsize, 0, ( struct sockaddr* )&m_dest_addr, sizeof( m_dest_addr ) )
			< 0 )
		{
			perror( "sendto error" );
			continue;
...
bool Ping::recvPacket( PingResult& pingResult )
{
	...
	for( int recvCount = 0; recvCount < m_maxPacketSize; recvCount++ )
	{
		...
		{
			if( ( len = recvfrom(
					  m_sockfd, m_recvpacket, sizeof( m_recvpacket ), 0, ( struct sockaddr* )&m_from_addr, &fromlen ) )
				< 0 )
			{
				if( errno == EINTR )
					continue;
